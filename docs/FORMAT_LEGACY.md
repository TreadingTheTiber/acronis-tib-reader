...
`/path/to/legacy_example.tib` (8 GB, January 2014) and confirmed
...
- The TI 2018+ binary recognises **two distinct legacy generations**:
  1. **`SequentialImageStream`** — reads TI 2014/15/16 sector-mode `.tib`
     ...
     blob lacks **TLV tag `0x9b`**. This is the path example takes.
  2. **`ConvertFromLegacyFormat`** (`container_convert.cpp`) — converts an
     ...
     **Example fails this test** (its `version==0` and `+0x1c==0x20`), so it
...
Example has `version=0` and `+0x1c=0x20`, so it FAILS this gate — meaning it
...
Confirmed empirically against example: stored Adler32 `0xA42A071E` validates.

The `version` field at +0x06 is **not** what triggers the legacy reader.
example has `version=0` and is read as legacy. Per the version-field
convention (0=Win, 1=Mac), this is a Windows-source legacy archive.

The trailing `+0x1C` u32 in example is `0x20` (32). In the modern format
...
Empirically confirmed against example; matches the on-disk pattern reported
by the user.
...
= 16 bytes. Empirically confirmed for example: first block at offset 32 has
...
filesize -48 ..  -33    16 bytes:
  ...
  +8  u32 metaDataOffset    (= 0x0b23513e in example)
  +12 u32 fullSize_or_count (= 0x02 in example)
...
The ~60 KB chunk-map size for example is plausible: 8 GB / 256 KiB = 32 K
...
| `0x091f6780` | `ConvertFromLegacyFormat` | `container_convert.cpp` | **Different (even older) legacy** — converts ancient containers in-place. Gate: `version==1`, `+0x1c==0x1000`. **Does NOT apply to TI 2014/example**. |
...
| Example is read via SequentialImageStream, not ConvertFromLegacyFormat | **Decompiled** + empirical (header values fail ConvertFromLegacy gate) |
| 8-byte preamble, 64-cluster blocks | **Empirical** (example first block decompresses cleanly) |
| Clusters-per-block stored in TLV tag 3 | **Decompiled** (FUN_08982090 stores tag 3 at stream `[0xC]`, which is the bitmap multiplier) |
| Mini-trailer layout (16 B before mirror, no 0x94E18A2B magic) | **Empirical** (example trailer dump) |
...
| Chunk map size ~60 KB at end of example file | **Empirical** (1KB-bucket density profile) |
