...
empirically verified on `example.tibx` (header v8) that:
...
empirical schema bytes from `example.tibx` (`hdr_size=0x1540`,
header_version=8).

### Confidence

- **Confirmed via decompilation**: 0..2, 5..9, 11, 12, 13, 18.
- **Confirmed via empirical schema bytes** (`example.tibx`):
...
### Empirical TLV[9] decode (`example.tibx`, 130 bytes)
...
Decoded slots from `example.tibx`:
...
### Empirical TLV[18] decode (`example.tibx`, 12 bytes)
